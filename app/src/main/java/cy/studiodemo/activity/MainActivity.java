...
import cy.studiodemo.fragment.CarFragment;
import cy.studiodemo.fragment.VideoFragment;
...
import cy.studiodemo.fragment.GroupBuyFragment;
...
public class MainActivity extends BaseActivity implements View.OnClickListener {
    ...
    private String[] mLeftMenus = {"团购", "app下载", "聊天", "看视频", "汽车", "设置"};
    ...
    private void initFragments() {
        mFragments.add(new GroupBuyFragment());
    ...
    private void notifyCityChanged() {
        GroupBuyFragment groupBuyingFragment = (GroupBuyFragment) mFragments.get(0);
    ...
    private ArrayList getMenus() {
        ...
        menuItems.add(new MenuItem(mLeftMenus[4], R.mipmap.ic_dashboard));
        menuItems.add(new MenuItem(mLeftMenus[5], R.mipmap.ic_event));
        return menuItems;
    }

    private void setTitleName(int position) {
        tv_title_name.setText(mLeftMenus[position]);
    }
                ...
                switch (mPosition) {
                    case 0:
                        if ((System.currentTimeMillis() - mExitTime) > 2000) {
                            ...
                            VideoFragment videoFragment = (VideoFragment) mFragments.get(3);
                    ...
                    case 3:
                        VideoFragment videoFragment = (VideoFragment) mFragments.get(3);
                        if (isBigScreen) {
                            videoFragment.goSmallScreen();
                            setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                        } else {
                            videoFragment.pausePlay();
                            setTitleName(0);
                            switchContent(0);
                        }
                        break;
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        if (videoFragment != null) {
            if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
                videoFragment.onBigScreen();
                isBigScreen = true;
            } else if (newConfig.orientation == Configuration.ORIENTATION_PORTRAIT) {
                videoFragment.onSmallScreen();
                isBigScreen = false;
            }
        }
